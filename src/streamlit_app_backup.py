...
def runtime_grounded_answer(question: str, top_k: int = 5):
    ...
    # ----------------------------
    RELEVANCE_DISTANCE_THRESHOLD = 0.90
    ...
    if best_distance > RELEVANCE_DISTANCE_THRESHOLD:
        # 1) Build fallback tokens from the ORIGINAL user question
        q = question.lower()

        # keep useful tokens incl. math-like tokens; drop tiny/noise words
        raw_tokens = re.findall(r"[a-zA-Z0-9Δ∆]+", q)

        stop = {
            "does","the","paper","explicitly","present","show","give","provide","state",
            "calculate","using","use","for","to","of","in","a","an","and","or","is","are",
            "what","which","how","equation"
        }
        tokens = [t for t in raw_tokens if len(t) >= 3 and t not in stop]

        # If user wrote almost nothing, we cannot rescue safely
        if not tokens:
            return {
                "question": question,
                "answer": FALLBACK_MSG,
        ...
        # 2) Scan indexed chunks deterministically using those tokens
        texts = st.session_state.get("runtime_texts", [])
        metadatas = st.session_state.get("runtime_metadatas", [])
        ...
        for i, t in enumerate(texts):
            ...
            hit_count = sum(1 for tok in tokens if tok in tl)
            if hit_count >= 2:
                md = metadatas[i] if i < len(metadatas) else {}
                rescued.append({
                    "rank": len(rescued) + 1,
                    "distance": 0.0,  # explicit token match -> treat as relevant
                    "text": t,
                    "source_file": md.get("source_file"),
                    "page": md.get("page"),
                })
